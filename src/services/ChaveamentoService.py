        ...
        for categoria in categorias:
            async with async_session() as session:
                ...
                if comps:
                    ...
                    jogos_fem_serializable = [jogo.to_dict() for jogo in jogos_fem]
                    jogos_masc_serializable = [jogo.to_dict() for jogo in jogos_masc]

                    chaveamento = {
                        "categoria": categoria,
                        "chaves_fem": jogos_fem_serializable,
                        "chaves_masc": jogos_masc_serializable
                    }

                    resultados.append(chaveamento)

                    todos_competidores.extend(comps)

        if todos_competidores: 
            resultados.append({"competidores_categoria": [competidor.to_dict() for competidor in todos_competidores]})
